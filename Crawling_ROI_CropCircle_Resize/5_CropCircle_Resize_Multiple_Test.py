...
def CreateDir(dir_name):
    try:
        if not os.path.exists("./crop_resize_data"):
            os.mkdir("./crop_resize_data")

        # 키워드 폴더 만들기
        if not os.path.exists("./crop_resize_data/{}".format(dir_name)):
            os.mkdir("./crop_resize_data/{}".format(dir_name))
    except OSError:
...
def Crop_Img(target):
    ...
    for file_name in file_list:
        ...
        try:
            ...
            print("Can't upload image in : {}/{} !!!".format(target, file_name))
            continue
        ...
        circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, max(dst_h, dst_w), param1=250, param2=10,
                                   minRadius=int(dst_h / 8), maxRadius=int(dst_h))

        # 근사 원이 존재하지 않는 경우 : 다음 for 루프 반복
        if circles is None:
            print("Doesn't have circle in : {}/{}.png !!!".format(target, file_name))  # 확인용 코드
            continue
        ...
        for circle in circles[0]:
            ...
            write_path = "./crop_resize_data/{}/{}.png".format(target, pure_file_name)

            # ROI 영역 파일 resize
            roi_resize = cv2.resize(roi, dsize=(224, 224), interpolation=cv2.INTER_AREA)
            ...
            result, encoded_img = cv2.imencode(extension, roi_resize)

            if result:
                with open(write_path, mode='w+b') as f:
                    encoded_img.tofile(f)
            print("Image cropped + resized : {}/{}.png".format(target, pure_file_name))  # 확인용 코드
